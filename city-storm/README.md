...
- `_runtest.mjs` — headless smoke-test runner (`node _runtest.mjs`)
  Boots `game.js` against the mocks and runs 900 frames, asserting the wave,
  ammo and enemy state stay sane. The mocks cover the render pipeline too — a
  post chain that silently falls back would otherwise pass unnoticed — so a new
  Three.js call in the game or in the photoreal kit needs adding there first.
  CI runs it before building the APK.
